    ...
    <div class="appbanner-container">
      <div class="box-header">
        <el-button class="btn-add" type="primary" icon="el-icon-plus" size="small" @click.native="handleAdd">新增</el-button>
            <el-input v-model="la_name" placeholder="请输入类别名称" class="handle-input2 mr10" size="small"></el-input>
...
<script>
import {getLbclassList, deletelbClass, hidelbClass, editlbClass} from '@/http/api/peiyinRequest'
export default {
  name: 'pylabelManage',
  data () {
    return {
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {//更改字段名！！
        getLbclassList(this.page, this.la_name).then(response => {
    ...
    handleDelete () {
      ...
      return new Promise((resolve, reject) => {
        deletelbClass(pid).then(response => {
          if (response) {
            this.getData()
    ...
    hideEdit (index, row) {
      ...
      return new Promise((resolve, reject) => {
        hidelbClass({is_hide:row.is_hide, tags_id:row.id}).then(response => {
          if (response) {
            this.getData()
    ...
    handleEdit (index, row) {
      this.form['tags_id'] = row.id
      this.form.name = row.name //以下各项改成将要显示的字段名
      this.form.options = row.weight
